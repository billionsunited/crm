...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto" x-data="{ exportModalOpen: false, zipModalOpen: false }">
        ...
        <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 mb-6 mt-6">
            <form action="{{ route('invoices.index') }}" method="GET" class="flex flex-col md:flex-row items-end gap-4">
                <!-- Search Input -->
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('invoices.index') }}" class="h-[42px] px-6 bg-slate-100 text-slate-700 rounded-lg font-semibold hover:bg-slate-200 transition-colors flex items-center text-sm">
                        Clear
                    </a>
                </div>
            </form>
            ...
            <div id="table-container" class="overflow-x-auto w-full">
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    @forelse($invoices as $invoice)
                        <tr @click="window.location='{{ route('invoices.show', [$invoice->id, 'page' => $invoices->currentPage()]) }}'" class="hover:bg-slate-50 transition-colors group cursor-pointer" style="cursor: pointer;">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm font-medium text-slate-900 border border-slate-200 bg-white shadow-sm px-2 py-1 rounded-md">
                                    ...
                                    @else
                                        {{ $invoice->is_paid ? $invoice->invoice_number : 'PROFORMA' }}
                                    @endif
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium" @click.stop>
                                <div class="flex items-center justify-end gap-3">
                                    ...
                                    <a href="{{ route('invoices.show', [$invoice->id, 'page' => $invoices->currentPage()]) }}" class="text-blue-600 hover:text-blue-900 bg-blue-50 px-2 py-1 rounded-md hover:bg-blue-100 transition-colors">View</a>
                                    @endif
                                    
                                    @if(!$invoice->is_cancelled)
                                        @if($invoice->is_paid)
                                            @if(auth()->user()->isAdmin() || auth()->user()->can('invoice-edit'))
                                                <a href="{{ route('invoices.edit', [$invoice->id, 'page' => $invoices->currentPage()]) }}" class="text-amber-600 hover:text-amber-900 bg-amber-50 px-2 py-1 rounded-md hover:bg-amber-100 transition-colors">Edit</a>
                                            @endif
                                            @if(auth()->user()->isAdmin())
                                                <a href="{{ route('invoices.download', $invoice->id) }}" class="text-emerald-600 hover:text-emerald-900 bg-emerald-50 px-2 py-1 rounded-md hover:bg-emerald-100 transition-colors">Download</a>
                                            @endif
                                        @else
                                    ...
                                    @else
                                        @if(auth()->user()->isAdmin() || auth()->user()->can('invoice-delete'))
                                            <form action="{{ route('invoices.destroy', $invoice->id) }}" method="POST" class="inline-block m-0">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('Are you sure you want to permanently delete this cancelled invoice? This action cannot be undone.')" class="text-red-600 hover:text-red-900 bg-red-50 px-2 py-1 rounded-md hover:bg-red-100 transition-colors">Delete</button>
                                            </form>
                                        @endif
                                    @endif
                                </div>
                        ...
                        <tr>
                            <td colspan="7" class="px-6">
                                <div class="flex flex-col items-center justify-center py-12 text-center gap-3">
                                    ...
                                    <h3 class="text-sm font-semibold text-slate-900 border-none pb-0">No Invoices Found</h3>
                                    <p class="text-sm text-slate-500 max-w-sm">No invoices have been generated yet.</p>
                                    <div>
                                        <a href="{{ route('invoices.create') }}" class="inline-flex items-center px-6 h-12 border border-transparent shadow-sm font-semibold rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
        ...
        <div x-show="exportModalOpen" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6" style="display: none;" x-cloak>
            ...
            <div x-show="exportModalOpen" x-transition:enter="transition ease-out duration-300"
                ...
                <!-- Modal Form -->
                <form action="{{ route('invoices.export') }}" method="GET" @submit="exportModalOpen = false">
