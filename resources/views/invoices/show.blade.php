        ...
        <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-5xl mx-auto">
            ...
            <div class="sm:flex sm:justify-between sm:items-center mb-8 no-print">
                ...
                <div class="flex flex-nowrap items-center gap-3 overflow-x-auto pb-2 sm:pb-0">
                    <a href="{{ route('invoices.index', array_filter(['page' => request('page')])) }}"
                    ...
                    @if(!$invoice->is_cancelled)
                        @if(auth()->user()->isAdmin() && !$invoice->is_paid)
                            <form action="{{ route('invoices.mark_paid', $invoice->id) }}" method="POST" class="inline-block m-0">
                                @csrf
                                <button type="submit"
                            ...
                            <form action="{{ route('invoices.cancel', $invoice->id) }}" method="POST" class="inline-block m-0">
                                @csrf
                                <button type="submit"
                        ...
                        @can('email-section')
                            <form action="{{ route('invoices.send_email', $invoice->id) }}" method="POST" class="inline-block m-0">
                                @csrf
                                <button type="submit"
            ...
            <div class="w-full overflow-x-auto print:overflow-visible pb-4">
                <div class="bg-white shadow-2xl rounded-sm border border-slate-200 mx-auto overflow-hidden print:shadow-none print:border-none print:m-0 print:p-0"
                    ...
                    <!-- Top Header Info -->
                    <table class="w-full mb-2 header-info-tbl">
                        <tr>
                            <td style="width: 50%;"></td>
                            <td style="width: 50%; text-align: left; padding-left: 40px;">
                                <div class="mb-1"><span class="w-bold">GSTIN:</span> <span
                                        class="w-bold">29AKNPG5479L1ZB</span>
                                </div>
                                <div class="mb-1"><span class="w-bold">Name:</span> <span class="w-bold uppercase">Billions
                                        United</span></div>
                                <div class="mb-1"><span class="w-bold">Address:</span> <span class="w-normal">#35, 1st
                                        Floor,
                                        24th
                                ...
                                <div class="mb-1"><span class="w-bold">Contact Details:</span> <span class="w-normal">+91
                                        8048516757</span></div>
                                <div class="mb-1"><a href="https://www.billionsunited.com"
                                        class="w-bold underline text-black">www.billionsunited.com</a></div>
                            </td>
                        </tr>
                    </table>

                    <div class="w-bold text-center uppercase border border-black py-1 mb-0"
                        style="font-size: 13px; border-bottom: none;">{{ $invoice->is_paid ? 'INVOICE' : 'PROFORMA' }}</div>

                    <table class="paper-invoice-table">
                        <tr>
                            <td style="width: 50%; padding: 8px;">
                                <div class="w-bold underline mb-2 uppercase">Details of Receiver (Billed to)</div>
                                ...
                                <div class="mb-1"><span class="w-bold">Udyam Est -</span> <span
                            ...
                            <td style="width: 50%; padding: 0;">
                                <table class="w-full meta-table-web">
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>:</td>
                                        <td class="w-normal">{{ $invoice->service_description_meta ?: 'SMS & WhatsApp' }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="w-bold">Invoice Number</td>
                                        <td>:</td>
                                        <td class="w-normal">{{ $invoice->is_paid ? $invoice->invoice_number : 'PROFORMA' }}
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>:</td>
                                        <td class="w-normal">
                                            {{ $invoice->due_date ? $invoice->due_date->format('d-M-y') : 'Immediate' }}
                                        </td>
                                    </tr>
                                    <tr>
                        ...
                        <tbody>
                            @foreach($invoice->items as $index => $item)
                                @php
                                    ...
                                    $cgstPercent = isset($invoice->cgst_percent) && $invoice->cgst_percent !== null ? (float)$invoice->cgst_percent : ($invoice->tax_type === 'local' ? 9.0 : 0.0);
                                    $sgstPercent = isset($invoice->sgst_percent) && $invoice->sgst_percent !== null ? (float)$invoice->sgst_percent : ($invoice->tax_type === 'local' ? 9.0 : 0.0);
                                    $igstPercent = isset($invoice->igst_percent) && $invoice->igst_percent !== null ? (float)$invoice->igst_percent : ($invoice->tax_type === 'outstation' ? 18.0 : 0.0);
                                ...
                                <tr>
                                    ...
                                    <td class="text-center w-normal">{{ $item->hsn_sac ?? '998599' }}</td>
                    ...
                    <table class="w-full mt-8">
                        <tr>
                            <td style="width: 65%; vertical-align: top;">
                                <div class="mb-3"><span class="w-bold">Declaration:</span> <span
                                        class="w-normal uppercase italic">NA</span></div>
                                <div><span class="w-bold">Signatory:</span> <span class="w-normal">Billions United</span>
                                </div>
                            </td>
                            <td style="width: 35%; text-align: left; vertical-align: top;">
                                <div class="w-bold mb-1">For Billions United</div>
                                <div class="mb-1">
                                    @php
                                    ...
                                    @if (file_exists($signaturePathWeb))
                                        <div class="mb-1">
                                            <img class="signature-img-web" src="{{ asset('signature.png') }}" alt="Signature">
                                        </div>
                                    @endif
                                </div><br>
                                <div style="line-height: 1.25;">
                                    <div class="w-bold uppercase">Authorized Signatory</div>
                                    <div><span class="w-bold">Name:</span> <span class="w-normal">Billions United</span>
                                    </div>
                                    <div><span class="w-bold">Designation / Status:</span> <span
                    ...
                    <div class="mt-6 pt-3 border-t border-black text-[11px] leading-tight w-normal">
                        ...
                        Note: Client shall NOT Resell, sublicense, lease, distribute, publish our services. Client shall not
                        use
                        our services for phishing, impersonation, fraud, financial scams, digital arrest or cybercrime. T&C
                        is on MSA. Client shall irrevocably and unconditionally defend, indemnify and hold harmless Billions
                        United. Billions United, in its sole discretion, reserves the right to disconnect the services
                        without any notice.
                    </div>

                    <div class="mt-5 text-center w-bold" style="font-size: 13px;">
                        This is a <span class="underline">Computer Generated</span>
                        {{ $invoice->is_paid ? 'Invoice' : 'Proforma' }}
                    </div>
                </div>
            </div>
        </div>
