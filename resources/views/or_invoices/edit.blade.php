@extends('layouts.app')
@section('header', 'Edit OR Invoice')
@section('content')
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto" x-data="invoiceForm()">
        ...
        <div class="mb-8 flex flex-col md:flex-row gap-4 justify-between md:items-center">
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Edit Invoice OR ✨</h1>
            <a href="{{ route('or-invoices.show', [$invoice->id, 'page' => request('page')]) }}"
        ...
        <form action="{{ route('or-invoices.update', [$invoice->id, 'page' => request('page')]) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white shadow-sm border border-slate-200 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="date" name="due_date" value="{{ $invoice->due_date ? $invoice->due_date->format('Y-m-d') : date('Y-m-d') }}" required
            ...
            </div>            <!-- Section: Services Configurator -->
            <div class="bg-white shadow-sm border border-slate-200 rounded-xl p-6">
                ...
                <div class="relative z-10">
                    ...
                    <div class="space-y-6 font-mono text-lg">
                        ...
                        <div x-show="taxType === 'local'" class="flex justify-between text-indigo-300">
                            <span class="uppercase tracking-widest text-xs font-bold leading-relaxed px-1">CGST (1.5%)</span>
                            <span class="font-bold"
                                x-text="'₹ ' + (taxableValue * 0.015).toLocaleString('en-IN', {minimumFractionDigits: 2})"></span>
                        </div>

                        <div x-show="taxType === 'local'" class="flex justify-between text-indigo-300">
                            <span class="uppercase tracking-widest text-xs font-bold leading-relaxed px-1">SGST (1.5%)</span>
                            <span class="font-bold"
                                x-text="'₹ ' + (taxableValue * 0.015).toLocaleString('en-IN', {minimumFractionDigits: 2})"></span>
                        </div>
                        ...
                        <div x-show="taxType === 'outstation'" class="flex justify-between text-rose-300"
                            style="display: none;">
                            <span class="uppercase tracking-widest text-xs font-bold leading-relaxed px-1">IGST (3%)</span>
                            <span class="font-bold"
                                x-text="'₹ ' + (taxableValue * 0.03).toLocaleString('en-IN', {minimumFractionDigits: 2})"></span>
                        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('invoiceForm', () => ({
                ...
                selectedLeadId: '{{ $invoice->lead_id }}',
                serviceRates: {
                    "Design Services": 5000,
                    "Web Development": 15000,
                    "Consultation": 2000,
                    "Server Maintenance": 3000
                },
                receiver: {
                    client_name: {!! json_encode($invoice->client_name) !!},
                    organisation_name: {!! json_encode($invoice->organisation_name) !!},
                    address: {!! json_encode(str_replace(["\r", "\n"], " ", $invoice->address)) !!},
                    city: {!! json_encode($invoice->city) !!},
                    state: {!! json_encode($invoice->state) !!},
                    state_code: {!! json_encode($invoice->state_code) !!},
                    udyam_certificate: {!! json_encode($invoice->udyam_certificate) !!},
                    pan_no: {!! json_encode($invoice->pan_no) !!},
                    aadhar_no: {!! json_encode($invoice->aadhar_no) !!},
                    gstin_unique_id: {!! json_encode($invoice->gstin_unique_id) !!},
                    purchase_order: {!! json_encode($invoice->purchase_order) !!},
                },

                items: {!! $invoice->items->count() > 0 ? $invoice->items->map(function($i) { 
                    $predefined = ['Design Services', 'Web Development', 'Consultation', 'Server Maintenance'];
                    $isOther = !in_array($i->service_name, $predefined);
                    return ['service_name' => $isOther ? 'Other' : $i->service_name, 'custom_name' => $isOther ? $i->service_name : '', 'hsn_sac' => $i->hsn_sac, 'qty' => $i->qty, 'rate' => $i->rate, 'total' => $i->total, 'calculatedMode' => 'auto']; 
                })->toJson() : json_encode([['service_name' => '', 'custom_name' => '', 'hsn_sac' => '7117', 'qty' => '', 'rate' => '', 'total' => $invoice->taxable_value, 'calculatedMode' => 'auto']]) !!},
                ...
                handleServiceSelection(index) {
                    ...
                    if (item.service_name && this.serviceRates[item.service_name] !== undefined) {
                        item.rate = '';
                        item.qty = '';
                ...
                addItem() {
                    this.items.push({ service_name: '', custom_name: '', hsn_sac: '7117', qty: '', rate: '', total: '', calculatedMode: 'auto' });
                },
                ...
                calculateGrandTotal() {
                    ...
                    const taxMultiplier = this.taxType === 'local' ? 0.03 : 0.03; // Local (CGST 1.5% + SGST 1.5%) vs Outstation (IGST 3%)
                ...
                get serviceDescriptionMeta() {
                    ...
                    if (names.length === 0) return 'Design Services';
