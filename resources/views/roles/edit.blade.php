...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-5xl mx-auto">
    ...
    <div class="mb-8 flex justify-between items-center mobile-edit-header">
        <div>
            <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Edit Role: <span class="text-indigo-600">{{ $role->name }}</span> ✨</h1>
            <p class="text-sm text-slate-500 mt-1">Modify role permissions and update configuration for this system role.</p>
        </div>
        ...
        <div class="bg-white shadow-sm border border-slate-200 rounded-xl p-6">
            ...
            <div class="max-w-md">
                ...
                <input id="name" name="name" 
                    class="w-full rounded-lg border-slate-200 text-sm focus:border-indigo-500 focus:ring-indigo-500 h-[42px] px-3 bg-white {{ $role->name === 'admin' ? 'bg-slate-50 cursor-not-allowed opacity-75' : '' }}" 
                    type="text" placeholder="e.g. Sales Manager" required value="{{ old('name', $role->name) }}" {{ $role->name === 'admin' ? 'readonly' : '' }} />
                @if($role->name === 'admin')
                    <p class="mt-2 text-xs text-amber-600 font-medium flex items-center">
                        ...
                        The 'admin' role name cannot be modified.
                    </p>
                @endif
            </div>
        </div>
