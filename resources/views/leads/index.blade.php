...
<div class="relative">
    ...
    <div x-data="{
        ...
        <div class="flex items-start justify-between gap-4 mb-6" style="flex-wrap: wrap;">
            <div>
                <h1 class="text-xl font-semibold text-slate-900 tracking-tight">Leads Pipeline</h1>
                <p class="text-sm text-slate-500 mt-1">Track and manage prospective clients, deals, and follow-ups.</p>
            </div>
                 ...
                 <div class="leads-actions-row w-full sm:w-auto">
                     ...
                     @can('lead-add')
                         <a href="{{ route('leads.create') }}"
                             class="inline-flex items-center justify-center px-6 h-12 border border-transparent rounded-lg shadow-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors whitespace-nowrap add-lead-btn">
                             Add Lead
                         </a>
                     @endcan
                 </div>
 
                  <!-- Bulk Messaging Action Bar: Visible only when leads are selected via checkboxes -->
                  <div x-show="selectedLids.length > 0" x-transition
                      ...
                      <div class="leads-bulk-bar-buttons flex items-center gap-2">
                          <!-- Open Messaging Modal for Campaign -->
                          ...
                          @can('lead-delete')
                              <button type="button"
                                  @click="if(confirm('Are you sure you want to delete ' + selectedLids.length + ' selected leads? This action cannot be undone.')) { $refs.bulkDeleteForm.submit() }"
                              ...
                              <form x-ref="bulkDeleteForm" action="{{ route('leads.bulk_destroy') }}" method="POST"
                                  class="hidden">
                                  @csrf
        ...
        <div x-data="topScrollHandler()" x-init="init()" class="flex flex-col gap-2 mt-6">
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex-1">
                <div x-ref="contentContainer" @scroll="sync($el, $refs.topScrollbar)" class="overflow-x-auto h-full">
                    ...
                    <thead class="bg-slate-50/80 backdrop-blur-sm sticky top-0 z-1">
                        <tr>
                            ...
                            <th scope="col"
                                class="px-4 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">
                                Record ID</th>
                            <th scope="col"
                                class="px-4 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">
                                Lead Information</th>
                        ...
                        @forelse($leads as $lead)
                            <tr @click="window.location='{{ route('leads.show', [$lead->id, 'page' => request('page')]) }}'"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap" @click.stop>
                                    <div class="flex flex-col items-start w-max gap-1">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2 mb-0.5">
                                        ...
                                        @php $docCount = $lead->documentTrackings()->count(); @endphp
                                        @if($docCount > 0)
                                            <span class="flex items-center justify-center w-5 h-5 text-[10px] font-bold text-white bg-indigo-600 rounded-full shrink-0" title="Documents Sent">
                                                {{ $docCount }}
                                            </span>
                                        @endif
                                    </div>
                                    @if($lead->contact_person)
                                        <div class="text-[11px] text-indigo-600 font-medium italic">
                                            Contact: {{ $lead->contact_person }}
                                        </div>
                                    @endif

                                    <div class="text-xs text-slate-600 truncate max-w-[150px]"
                                    ...
                                    <div class="mt-1.5 flex flex-col gap-1.5">
                                        ...
                                        @if($lead->original_id && $lead->original_id !== $lead->id)
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium w-max bg-rose-100 text-rose-800">
                                                Duplicate of #{{ $lead->original_record_id }}
                                            </span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="text-sm text-slate-600 truncate max-w-[150px]"
                                    ...
                                    <div class="flex items-center justify-end gap-3">
                                        ...
                                        @if($lead->creation_source === 'CLIENT REGISTRATION')
                                            @if($lead->is_agreement_sent)
                                                <span
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-100 text-indigo-700 border border-indigo-200">
                                                    Agreement Sent
                                                </span>
                                            @else
                                                <form action="{{ route('leads.send_agreement', $lead->id) }}" method="POST"
                                                    class="inline">
                                                    @csrf
                                                    <button type="submit"
                                                        class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 px-2 py-1 rounded-md hover:bg-indigo-100 transition-colors flex items-center gap-1"
