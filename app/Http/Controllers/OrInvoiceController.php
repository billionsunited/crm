...
class OrInvoiceController extends Controller
{
    private function enforceOrType(Invoice $invoice)
    {
        if ($invoice->invoice_per_type !== 'or') {
            abort(404);
        }
    }
    ...
    {
        $query = Invoice::with('lead')->where('invoice_per_type', 'or')->latest();
        ...
        $clients = Invoice::where('invoice_per_type', 'or')
        ...
        return view('or_invoices.index', compact('invoices', 'clients'));
    }
    ...
    public function export(Request $request)
    {
        ...
        $fileName = "or-invoices-export-{$monthName}-{$year}.csv";
        ...
        $callback = function () use ($query, $columns) {
            ...
            $query->chunk(100, function ($invoices) use ($file) {
                foreach ($invoices as $invoice) {
                    ...
                    // Join unique HSN/SAC codes, fallback to 7117 if empty
                    ...
                    $row = [
                        ...
                        '=HYPERLINK("' . URL::signedRoute('or-invoices.download-page', ['invoice' => $invoice->id]) . '","Download Invoice")',
                    ];
                    fputcsv($file, $row);
                }
            });
    ...
    public function store(Request $request)
    {
        ...
        if ($request->tax_type === 'local') {
            $cgst_amount = round($taxableValue * 0.015, 2);
            $sgst_amount = round($taxableValue * 0.015, 2);
        } else {
            // IGST for OR invoices is 3%
            $igst_amount = round($taxableValue * 0.03, 2);
        }
        ...
        $invoice = Invoice::create([
            ...
            'service_description_meta' => $request->service_description_meta ?? 'Design Services',
            ...
            'cgst_percent' => ($request->tax_type === 'local' ? 1.5 : 0),
            'cgst_amount' => $this->formatNumber($cgst_amount),
            'sgst_percent' => ($request->tax_type === 'local' ? 1.5 : 0),
            'sgst_amount' => $this->formatNumber($sgst_amount),
            'igst_percent' => ($request->tax_type === 'outstation' ? 3 : 0),
        ...
        return redirect()->route('or-invoices.show', $invoice->id)->with('success', 'Invoice generated successfully.');
    }

    public function show(Invoice $invoice)
    {
        $this->enforceOrType($invoice);
        $invoice->load('items');
        return view('or_invoices.show', compact('invoice'));
    }
    ...
    public function update(Request $request, Invoice $invoice)
    {
        ...
        return redirect()->route('or-invoices.show', [$invoice->id, 'page' => $request->query('page')])->with('success', 'Invoice updated successfully.');
    }
    ...
    public function download(Request $request, Invoice $invoice)
    {
        ...
        // Fallback: regenerate if file is missing from disk
        $pdf = Pdf::loadView('or_invoices.pdf', compact('invoice'));
    ...
    public function downloadPage(Request $request, Invoice $invoice)
    {
        ...
        $downloadUrl = URL::signedRoute('or-invoices.download', ['invoice' => $invoice->id]);
...
<body>
    ...
    public function markAsPaid(Invoice $invoice, InvoiceService $invoiceService)
    {
        ...
        if ($result['success']) {
            return redirect()->route('or-invoices.show', $invoice->id)->with('success', $result['message']);
        } else {
            return redirect()->route('or-invoices.show', $invoice->id)->with('error', $result['message']);
        }
    }
    ...
    public function cancel(Invoice $invoice)
    {
        ...
        try {
            DB::transaction(function () use ($invoice) {
                // 1. Generate sequence and update model securely as cancelled
                $invoice->assignFinalSequence(true);
            ...
            return redirect()->back()->with('success', 'Invoice cancelled and sequence generated successfully.');
        } catch (Exception $e) {
            Log::error('[Invoice System] Failed to cancel invoice', [
                'invoice_id' => $invoice->id,
                'error' => $e->getMessage()
            ]);
            return redirect()->back()->with('error', 'Failed to cancel invoice due to a system error.');
        }
    }
    ...
    public function destroy(Invoice $invoice)
    {
        ...
        return redirect()->route('or-invoices.index')->with('success', 'Invoice deleted successfully.');
    }
